...
- 明确 SIT 访问链接命名规则（在现网站点下使用 `sit` 子路径）。
...
2. SIT 页面链接规则：
   - 以现网链接为基准，新增 `/sit/` 子路径。
   - 例如：现网为 `https://<owner>.github.io/<repo>/`，SIT 为 `https://<owner>.github.io/<repo>/sit/`。
...
- [ ] SIT URL 符合“原链接 + `/sit/` 子路径”规则，且不覆盖现网页面。
...
1. SIT 页面采用同仓库子路径方案，固定为 `https://<owner>.github.io/<repo>/sit/`。
