...
- [ ] `pytest` 默认测试全部通过，不依赖网络
- [ ] `python scripts/fetch.py` 生成 `data/raw/YYYYMMDD_*.json`
- [ ] `python scripts/normalize.py` 生成 `data/normalized/YYYYMMDD_*.json`
- [ ] `python scripts/derive.py` 生成 `data/derived/models_YYYYMMDD_HHMMSS.csv` + `.json` + `models_latest.json`
- [ ] derived 字段齐全，行数与 raw `.data` 一致，`models_latest.json` 软链接指向最新时间戳 JSON 文件
- [ ] benchmark 允许部分为空；如果全部模型的 `intelligence_index`、`coding_index`、`agentic_index` 都为空，需标记为可疑并人工复核
- [ ] 抽取若干 derived 记录，用浏览器访问 OpenRouter 模型页，核对网页展示与提取数据一致（名称、context、价格、能力标签、benchmark）
- [ ] `ruff check .` 零报错
