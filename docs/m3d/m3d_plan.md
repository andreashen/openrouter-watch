...
1. **latest 对齐口径**：`~xxx/latest` 通过新增“指向关系”对齐到具体模型；前端基于该关系展示目标模型参数。
2. **多记录选取策略**：同一 slug 返回多条 benchmark 时，按 `Intelligence Index` 最高值选择目标记录（暂不维护 Effort Level）。
3. **`gpt-5.5-pro` 空指标处理**：平台返回空值时保持 `null`；若历史版本存在有效值，沿用历史值，不回退同族模型。
4. **前端开关默认值**：`隐藏 latest 标识模型` 默认开启；`隐藏已下架模型` 默认关闭。
5. **筛选持久化**：不写入 URL，改为写入 `localStorage`。
