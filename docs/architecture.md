...
公开主榜的三项指数仍来自 OpenRouter `benchmarks.artificial_analysis`，derive 仍按「新值覆盖、空白回填上一版」合并。直连 AA 的快照、映射报告写在 `data/aa_internal/`（不入库）。`data/aa_join/publication.json` 把公开展示固定为 `openrouter`，并继续采集 OpenRouter benchmark。
