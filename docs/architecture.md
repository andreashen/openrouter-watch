...
- **厂商快筛**：固定主流厂商按钮（非按数据动态扩缩）。芯片按 `vendor_name` 精确匹配，个别品牌可配置别名（例如 `xAI` 同时匹配 `SpaceXAI`，覆盖 OpenRouter 展示前缀更名）；多选为并集（OR）。
- **能力筛选**：Reasoning / Tools / Vision 复选，勾选即要求对应字段为 `true`。
- **数值范围**：上下文、最大输出、输入/加权输入/输出价、三项 index 的 min/max；该字段为 `null` 且设置了范围时过滤掉该行。
- **排序**：上述数值字段及知识截止 / 发布日期 / 更新时间；`null` 始终排最后；同值回退 `model_id`。排序字段与方向合并为单列复合触发器（S4：点击展开双栏面板选字段 / 方向），与「已下架 / latest / Batch模型」三态筛选同处一行四等分。
- **已下架模型**：按 `officially_removed` 控制显示（默认隐藏）；下架行可带视觉弱化 / 徽章。
- **模型系列 latest**：按 pointer / latest 判定控制显示（默认隐藏）。
- **Batch模型**：`model_id` 大小写不敏感地以 `:batch` 结尾时视为 batch；三态「不显示 / 显示 / 只显示」（默认不显示）；不加行内徽章。
- **模型 Pin（会话级）**：Model ID 左侧书签按钮（方案 C：描边 → 实心，幽灵底）。已 Pin 行豁免全部筛选、整组置顶；组内默认按 Pin 先后，可拖拽重排（6 点手柄为仅指针交互的装饰元素，不进入 Tab / 读屏操作语义；无键盘重排）；提供「清空全部 Pin」（无确认框）；刷新页面后清空。不写 `localStorage`。
...
公开主榜的三项指数仍来自 OpenRouter `benchmarks.artificial_analysis`，derive 仍按「新值覆盖、空白回填上一版」合并。直连 AA 的每次完整抓取按 `aa_snapshot_id` 写入 `data/aa_internal/snapshots/`（不入库、不覆盖已有文件），`latest_snapshot.json` 只保存指向该文件的指针。`data/aa_join/publication.json` 把公开展示固定为 `openrouter`，并继续采集 OpenRouter benchmark。Free / Pro 档位本身不是转载授权；`redistributable` 必须为真且未过期，公开切流才会放行。
