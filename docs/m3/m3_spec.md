...
- 部署入口仅展示完整 URL 文本，不展示“在线访问”等前缀文案。
- 部署入口模块需加宽，默认不截断 URL，确保地址可完整可见。
...
- [x] 主表仅展示 `model_id`（不再展示 `author/slug/vendor_name/name` 列）
- [x] 统一搜索框可用（匹配 `model_id/vendor_name/name`，与其他筛选 AND）
- [x] 能力复选筛选可用（Reasoning/Tools/Vision，AND）
- [x] 关键数值字段范围筛选可用（min/max，AND）
- [x] 关键数值字段排序可用（升/降/无，null 排最后）
- [x] **UI/UX 现代化升级**：实现科技化、美观、前沿的视觉效果（包含暗黑模式/科技感配色、毛玻璃效果、精致的排版与交互动画），并新增亮色/深色主题切换与记忆功能。
- [x] 顶部标题区新增“部署网址”入口与“仓库 Star 引导”入口（含图标跳转、文案提示、可访问性要求），且主标题为 `openrouter 模型信息表`、部署 URL 完整显示不截断
- [x] `cd web && npm run build` 无错误
