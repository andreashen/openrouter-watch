...
- [x] 将首页主标题文案更新为：`openrouter 模型信息表`。
- [x] 在 `web/src/pages/index.astro` 顶部标题区域新增“部署网址”链接位，链接固定为 `https://andreashen.github.io/openrouter-watch/`，并仅展示完整 URL 文本（不显示“在线访问”文案）。
- [x] 拉宽部署网址模块，确保 URL 默认完整可见（不截断）。
- [x] 在同一区域新增 **GitHub 图标** 按钮并跳转 `https://github.com/andreashen/openrouter-watch`，附带中文文案“给项目点个 Star”（不能仅图标）。
- [x] 两类外链统一补齐 `target="_blank"` 与 `rel="noopener noreferrer"`。
- [x] 校验亮/暗主题下的可读性与 hover/focus 反馈；键盘可聚焦触发（可访问性达标）。
